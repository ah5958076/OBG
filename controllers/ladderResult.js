...
const { LADDER_RESULT_ADDED, UNEXPECTED_ERROR, LADDER_RESULT_UPDATED, LADDER_RESULT_DELETED } = require("../constants/messages");
...
module.exports.delete = async (req, res) => {
    ...
    let value = await LadderResultModel.updateOne({_id: id}, givenObject).catch((e) => {console.log(e)});
...
module.exports.show = async (req, res) => {
    ...
    let id = req.params?.id || "";
    if(!id) return res.status(INVALID).send(makeResponse(UNEXPECTED_ERROR));
    let value = await LadderResultModel.findOne({_id: id})
...
module.exports.searchData = async (req, res) => {
    ...
    let population_fields = ["ladderId", "submittedBy", "teamId"];
    res.status(OK).send(makeResponse("", searchDataWithPopulate(LadderResultModel, filter, fields, population_fields)));
}