...
module.exports.store = async (req, res) => {
    if(req.file){
        if(ALLOWED_EXTENSIONS.toLowerCase().includes(req.file.originalname.split(".").pop().toLowerCase()))
            res.send(await service.store(req));
        else{
...
module.exports.delete = async (req, res) => {
    res.send(await service.delete(req.body?.name));
}
module.exports.show = async (req, res) => {
    res.send(await service.show(req.body?.name));
}
module.exports.list = async (req, res) => {
    let page_number = req.query.pageNum;
    if(!page_number)
        page_number=1;
    res.send(await service.list(page_number));
}

module.exports.searchData = async (req, res) => {
    res.send(await service.searchData(req.body.search));
}